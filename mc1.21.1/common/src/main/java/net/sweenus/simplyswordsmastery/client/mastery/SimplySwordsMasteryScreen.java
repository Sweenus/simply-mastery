...
import net.sweenus.simplyswordsmastery.client.mastery.ui.Anim;
import net.sweenus.simplyswordsmastery.client.mastery.ui.FrameClock;
import net.sweenus.simplyswordsmastery.client.mastery.ui.GlassButtonWidget;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryChrome;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryLayout;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryPalette;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryPaletteSource;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryShowcase;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryNodeState;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryPrompt;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryTheme;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryUiPolicy;
import net.sweenus.simplyswordsmastery.client.mastery.ui.MasteryUiSounds;
import net.sweenus.simplyswordsmastery.client.mastery.ui.NodeAnimators;
import net.sweenus.simplyswordsmastery.client.mastery.ui.UiDraw;
...
import net.sweenus.simplyswordsmastery.mastery.network.RequestProfileSyncPacket;
import net.sweenus.simplyswordsmastery.mastery.network.UnlockNodePacket;
...
public final class SimplySwordsMasteryScreen extends Screen {
    ...
    private float appliedBlend = Float.NaN;
    private int[] branchBaseRgb = new int[0];
    ...
    private GlassButtonWidget rewardsButton;
    ...
    protected void init() {
        ...
        backButton = addDrawableChild(new GlassButtonWidget(layout.backButtonX, buttonY,
                layout.backButtonWidth, buttonHeight, Text.translatable("screen.simplyswordsmastery.back"),
                button -> backToForge(), this::palette, accent));
        rewardsButton = addDrawableChild(new GlassButtonWidget(layout.rewardsButtonX, buttonY,
                layout.rewardsButtonWidth, buttonHeight, Text.translatable("screen.simplyswordsmastery.rewards"),
                button -> openRewards(), this::palette, accent));
        respecButton = addDrawableChild(new GlassButtonWidget(layout.respecButtonX, buttonY,
                layout.respecButtonWidth, buttonHeight, Text.translatable("screen.simplyswordsmastery.respec"),
                button -> confirmRespec(), this::palette, accent));
        closeButton = addDrawableChild(new GlassButtonWidget(layout.closeButtonX, buttonY,
                layout.closeButtonWidth, buttonHeight, Text.translatable("screen.simplyswordsmastery.close"),
                button -> close(), this::palette, accent, true));
        ...
        promptCancel = addSelectableChild(new GlassButtonWidget(0, 0, 60, buttonHeight,
                Text.translatable("screen.simplyswordsmastery.prompt.cancel"),
                button -> closePrompt(), this::palette, accent));
        promptConfirm = addSelectableChild(new GlassButtonWidget(0, 0, 60, buttonHeight,
                Text.translatable("screen.simplyswordsmastery.prompt.confirm"),
                button -> acceptPrompt(), this::palette, accent, true));
        if (prompt != null) {
            layoutPrompt();
        }
        applyPromptGating();
    ...
    private void drawItemTooltip(DrawContext context, int mouseX, int mouseY) {
        ...
        var tooltip = new ArrayList<>(getTooltipFromItem(client, stack));
        MasteryProfileRegistry.resolveClient(stack).ifPresent(profile -> {
            MasteryState state = state(profile);
            var policy = MasteryProfileRegistry.client().policy(profile.progressionGroupId());
            tooltip.add(Text.translatable("screen.simplyswordsmastery.progression.points",
                    state.earnedPoints(), policy.pointCap()));
            tooltip.add(state.earnedPoints() >= policy.pointCap()
                    ? Text.translatable("screen.simplyswordsmastery.progression.capped")
                    : Text.translatable("screen.simplyswordsmastery.progression.xp", state.masteryXp(),
                            policy.nextCost(state.earnedPoints())));
        });
    ...
    private void drawMasteryBackground(DrawContext context) {
        ...
        MasteryChrome.headerPanel(context, palette, layout.frame, accent, panels);
        MasteryChrome.bodyPanel(context, palette, layout.canvasLeft, layout.canvasTop, layout.canvasRight,
                layout.canvasBottom, accent, panels, true);
        MasteryChrome.showcasePanel(context, palette, layout.frame, accent, panels);
    }
    ...
    private void drawHeader(DrawContext context, MasteryProfile profile, MasteryState state) {
        ...
        if (roomy) {
            microLabel(context, Text.translatable(PersonalMasteryView.personal()
                    ? "screen.simplyswordsmastery.ownership.player" : "screen.simplyswordsmastery.ownership.weapon"), textLeft,
                    titleTop, palette.INK_MUTED, alpha);
        }
        Text headerName = UiDraw.fit(textRenderer, stack.getName(),
                Math.round((layout.headerContentRight - textLeft) / nameScale));
        UiDraw.text(context, textRenderer, headerName, textLeft, nameY,
                MasteryTheme.argb(palette.DISPLAY, alpha), nameScale, true);

        if (roomy) {
            int right = layout.rewardsButtonX - 6;
            int available = state.availablePoints(profile);
            int floor = textLeft + Math.round(textRenderer.getWidth(headerName) * nameScale) + 10;
            right = statCell(context, right, floor, ruleTop, ruleBottom,
                    Text.translatable("screen.simplyswordsmastery.header.mastery"),
                    masteryCount(profile, state), palette.INK, alpha, true);
            statCell(context, right, floor, ruleTop, ruleBottom,
                    Text.translatable("screen.simplyswordsmastery.header.available"),
                    Text.literal(String.format("%02d", available)),
                    available > 0 ? accent : palette.INK_FAINT, alpha, false);
        }
    }
    ...
    private int statCell(DrawContext context, int right, int floor, int ruleTop, int ruleBottom, Text label,
        ...
        return MasteryChrome.statCell(context, textRenderer, palette, layout.frame, right, floor, ruleTop,
                ruleBottom, label, value, valueRgb, alpha, rule);
    }

    private void microLabel(DrawContext context, Text label, float x, float y, int rgb, float alpha) {
        MasteryChrome.microLabel(context, textRenderer, label, x, y, rgb, alpha);
    }

    private void vRule(DrawContext context, int x, int top, int bottom, float alpha) {
        MasteryChrome.vRule(context, palette, x, top, bottom, alpha);
    }
    ...
    private void drawBranchRow(DrawContext context, MasteryProfile profile, MasteryState state, int lane,
        ...
        int nameRgb = sealed ? palette.INK_SOFT : palette.DISPLAY;
        ...
        if (cursor + 4 <= bottom) {
            if (sealed) {
                UiDraw.boxOutline(context, columnLeft, cursor, columnLeft + 3, cursor + 3, 1,
                        MasteryTheme.argb(palette.INK_MUTED, 0.9F * rowAlpha));
            } else {
        ...
        if (cursor + 8 <= bottom) {
            ...
            UiDraw.text(context, textRenderer, Text.literal(ownedText), columnLeft, cursor,
                    MasteryTheme.argb(sealed ? palette.INK_DIM : rgb, rowAlpha), 1.0F, true);
            UiDraw.text(context, textRenderer, Text.literal("/ " + total),
                    columnLeft + textRenderer.getWidth(ownedText) + 3, cursor + 1,
                    MasteryTheme.argb(palette.INK_FAINT, rowAlpha), MICRO_SCALE, false);
            cursor += 11;
        }
        if (cursor + 3 <= bottom) {
            UiDraw.segmentStrip(context, palette, columnLeft, cursor, columnWidth, 3, total,
            ...
            if (linked || flow > 0.01F) {
                ...
                if (linked) {
                    ...
                    UiDraw.pathRange(context, edge.path(), edge.points(), head - 20.0F, head, 2.0F,
                            MasteryTheme.argb(palette.emphasise(rgb, 0.55F), 0.7F * reveal));
                }
                ...
                UiDraw.pathDashed(context, edge.path(), edge.points(), edge.length(), limit,
                        3.0F, 3.0F, 0.0F, 1.0F,
                        MasteryTheme.argb(palette.LOCKED_DASH, 0.9F * reveal));
            }
        }
    }
    ...
    private void drawNode(DrawContext context, MasteryProfile profile, MasteryState state, int index) {
        ...
        switch (nodeState) {
            case OWNED -> {
                ...
                UiDraw.bevel(context, palette, x0, y0, x1, y1, capstone ? 2 : 1,
            ...
            case LOCKED, CONFLICT -> {
                context.fill(x0, y0, x1, y1, MasteryTheme.argb(palette.LOCKED_FILL, 0.95F * alpha));
                UiDraw.bevel(context, palette, x0, y0, x1, y1, 1, 0.4F * alpha);
                UiDraw.boxOutline(context, x0, y0, x1, y1, 1,
                        MasteryTheme.argb(palette.LOCKED_FRAME, 0.95F * alpha));
            }
        }
        if (capstone && nodeState != MasteryNodeState.LOCKED) {
            UiDraw.boxRing(context, cx, cy, half - 3.0F, 1.0F,
                    MasteryTheme.argb(nodeState == MasteryNodeState.OWNED
                            ? palette.ON_ACCENT : rgb, 0.45F * alpha));
        }
        ...
        if (hover > 0.01F && focus <= 0.01F) {
            UiDraw.boxRing(context, cx, cy, half + 1.0F, 1.0F,
                    MasteryTheme.argb(palette.TEXTURE_INK, 0.4F * hover * alpha));
        }
        if (focus > 0.01F) {
        ...
        int glyphRgb = switch (nodeState) {
            case OWNED -> palette.ON_ACCENT;
            case REACHABLE -> rgb;
            default -> palette.LOCKED_GLYPH;
        };
        ...
        if (burst > 0.0F) {
            ...
            UiDraw.boxRing(context, cx, cy, half * (1.0F + progress * 2.2F), 2.0F,
                    MasteryTheme.argb(palette.emphasise(rgb, 0.5F), 0.7F * burst));
    ...
    private void drawShowcase(DrawContext context, MasteryProfile profile, MasteryState state) {
        ...
        MasteryShowcase.Hover hover = MasteryShowcase.draw(context, textRenderer, palette, accentRgb(),
                layout.showcaseLeft, layout.showcaseTop, layout.showcaseRight, layout.showcaseBottom,
                displayStack(), profileName(profile), masteryCount(profile, state),
                Text.translatable("screen.simplyswordsmastery.header.mastery"),
                MasteryUiPolicy.maxInvestablePoints(profile,
                        MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap()),
                masteryMeter.value(), seconds(), motion(), appear);
    ...
    private void drawDetailCard(DrawContext context, MasteryProfile profile, MasteryState state,
        ...
        context.fill(cardX + 2, cardY + 3, cardX + cardWidth + 2, cardY + cardHeight + 3,
                MasteryTheme.argb(palette.SHADOW, 0.45F * alpha));
        context.fill(cardX, cardY, cardX + cardWidth, cardY + cardHeight,
                MasteryTheme.argb(palette.CARD, 0.97F * alpha));
        ...
        UiDraw.text(context, textRenderer, UiDraw.fit(textRenderer, Text.translatable(node.nameKey()),
                ...
                MasteryTheme.argb(palette.DISPLAY, alpha), 1.0F, true);
        ...
        if (node.capstone()) {
            UiDraw.rightText(context, textRenderer, Text.translatable("screen.simplyswordsmastery.capstone"),
        ...
        for (int i = 0; i < lineCount; i++) {
            UiDraw.text(context, textRenderer, lines.get(i), textX, cardY + 28 + i * 10,
                    MasteryTheme.argb(palette.BODY, 0.95F * alpha), 1.0F, false);
        }
        ...
        context.fill(cardX + 1, footerY - 4, cardX + cardWidth - 1, footerY - 3,
                MasteryTheme.argb(palette.RULE, alpha));
        Text costLabel = Text.translatable("screen.simplyswordsmastery.header.cost");
        microLabel(context, costLabel, textX, footerY - 1, palette.INK_MUTED, alpha);
        UiDraw.pips(context, palette, textX + Math.round(textRenderer.getWidth(costLabel) * MICRO_SCALE) + 5,
    ...
    private void drawStatus(DrawContext context) {
        ...
        int accent = accentRgb();
        MasteryChrome.statusRule(context, palette, layout.frame, appear);

        UnlockResult result = MasteryClientFeedback.result();
        if (result != null && MasteryChrome.feedback(context, textRenderer, palette, layout.frame,
                Text.translatable("message.simplyswordsmastery.unlock." + result.name().toLowerCase()),
                result == UnlockResult.SUCCESS ? accent : palette.ACCENT_HOT, feedbackTimer, motion())) {
            return;
        }
        ...
        Text unspent = Text.translatable("screen.simplyswordsmastery.unspent", available);
        UiDraw.rightText(context, textRenderer,
                ...
                MasteryTheme.argb(available > 0 ? accent : palette.INK_MUTED, 0.9F * appear),
                1.0F, false);
    }

    private int keyHint(DrawContext context, int x, int y, String key, String action, float appear) {
        return MasteryChrome.keyHint(context, textRenderer, palette, x, y, key, action, appear);
    }
    ...
    private void drawPrompt(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int accent = accentRgb();
        context.fill(0, 0, width, height, MasteryTheme.argb(palette.SCRIM, 0.72F * fade));
        ...
        context.fill(promptX + 3, promptY + 4, x1 + 3, y1 + 4, MasteryTheme.argb(palette.SHADOW, 0.5F * fade));
        context.fill(promptX, promptY, x1, y1, MasteryTheme.argb(palette.CARD, 0.98F * fade));
        ...
        UiDraw.text(context, textRenderer,
                ...
                MasteryTheme.argb(palette.DISPLAY, fade), 1.0F, true);
        context.fill(promptX + 1, promptY + 20, x1 - 1, promptY + 21,
                MasteryTheme.argb(palette.RULE, fade));
        for (int i = 0; i < promptLines.size(); i++) {
            UiDraw.text(context, textRenderer, promptLines.get(i), textX, promptY + 25 + i * 10,
                    MasteryTheme.argb(palette.BODY, 0.95F * fade), 1.0F, false);
        }
        int ruleY = promptY + promptHeight - 33;
        context.fill(promptX + 1, ruleY, x1 - 1, ruleY + 1, MasteryTheme.argb(palette.RULE, fade));
    ...
    protected void addScreenNarrations(NarrationMessageBuilder builder) {
        ...
        if (narrated >= 0 && narrated < profile.nodes().size()) {
            ...
            nodeMessage.put(NarrationPart.POSITION, Text.translatable("screen.simplyswordsmastery.narration.position",
                    narrated + 1, profile.nodes().size(), Text.translatable(branch.nameKey())));
            nodeMessage.put(NarrationPart.HINT, Text.translatable("screen.simplyswordsmastery.narration.details",
                    nodeStatusText(profile, current, node, visualState),
                    prerequisiteText(current, node), Text.translatable(node.descriptionKey()),
                    node.capstone() ? Text.translatable("screen.simplyswordsmastery.narration.capstone") : Text.empty()));
            nodeMessage.put(NarrationPart.USAGE, visualState == MasteryNodeState.REACHABLE
                    ? Text.translatable("screen.simplyswordsmastery.narration.usage.unlock")
                    : Text.translatable("screen.simplyswordsmastery.narration.usage.navigate"));
        } else {
            builder.put(NarrationPart.HINT, Text.translatable("screen.simplyswordsmastery.narration.overview"));
        }
        addElementNarrations(builder.nextMessage());
    }
    ...
    private void confirmUnlock(int index, MasteryProfile.Node node) {
        openPrompt(new MasteryPrompt(
                Text.translatable("screen.simplyswordsmastery.unlock.confirm.title",
                        Text.translatable(node.nameKey())),
                node.capstone()
                        ? Text.translatable("screen.simplyswordsmastery.unlock.confirm.capstone", node.cost())
                        : Text.translatable("screen.simplyswordsmastery.unlock.confirm.message", node.cost()),
                () -> sendUnlock(index)));
    }
    ...
    private void sendUnlock(int index) {
        ...
        new UnlockNodePacket(handler.syncId, definitionEpoch, state.mutationRevision(),
                profile.id(), node.id(), pendingActionId, weaponId(), PersonalMasteryView.mode()).sendToServer();
    }
    ...
    private void confirmRespec() {
        ...
        openPrompt(new MasteryPrompt(
                Text.translatable("screen.simplyswordsmastery.respec.confirm.title"),
                Text.translatable("screen.simplyswordsmastery.respec.confirm.message",
    ...
    private void requestRespec() {
        ...
        UnlockNodePacket.respec(handler.syncId, definitionEpoch, current.mutationRevision(),
                profile.id(), pendingActionId, weaponId(), PersonalMasteryView.mode()).sendToServer();
    }

    private void rebuildProfile(MasteryProfile replacement) {
        layout = new MasteryLayout(replacement, width, height);
        branchBaseRgb = new int[replacement.branches().size()];
        for (int i = 0; i < branchBaseRgb.length; i++) {
            branchBaseRgb[i] = replacement.branches().get(i).color() & 0xFFFFFF;
        }
        applyBranchColors();
    ...
    private MasteryState state(MasteryProfile profile) {
        return PersonalMasteryView.read(authoritativeStack(), profile,
                Math.min(MasteryConfig.SERVER.verticalSliceStartingPoints,
                        MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap()));
    }

    private UUID weaponId() {
        return authoritativeStack().getOrDefault(
                MasteryComponents.WEAPON_ID.get(), new UUID(0, 0));
    }
    ...
    private static Text masteryProgress(MasteryProfile profile, MasteryState state) {
        return Text.translatable("screen.simplyswordsmastery.mastery_progress", state.spentPoints(profile),
                MasteryUiPolicy.maxInvestablePoints(profile, MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap()));
    }
    ...
    private int accentRgb() {
        ...
        return branchRgb.length > 0 ? branchRgb[0] : palette.ACCENT;
    }
    ...
    private void refreshPalette() {
        if (paletteSource.refresh(displayStack())) {
            palette = paletteSource.palette();
            applyBranchColors();
        } else if (appliedBlend != MasteryConfig.CLIENT.branchAccentBlend) {
            applyBranchColors();
        }
    }

    private void applyBranchColors() {
        float blend = MasteryConfig.CLIENT.themedFromTooltips
                ? MasteryConfig.CLIENT.branchAccentBlend
                : 0.0F;
        appliedBlend = MasteryConfig.CLIENT.branchAccentBlend;
        branchRgb = palette.harmoniseBranches(branchBaseRgb, blend);
    }
    ...
    private static Text profileName(MasteryProfile profile) {
        return Text.translatable("profile.simplyswordsmastery." + profile.id().getPath());
    }

    private static float investedFraction(MasteryProfile profile, MasteryState state) {
        int total = MasteryUiPolicy.maxInvestablePoints(profile, MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap());
    ...
    private static Text nodeStatusText(MasteryProfile profile, MasteryState state, MasteryProfile.Node node,
        ...
        if (!MasteryConfig.SERVER.enabled || MasteryConfig.SERVER.disabledProfiles.contains(profile.id())) {
            return Text.translatable("screen.simplyswordsmastery.node_state.disabled");
        }
        if (nodeState == MasteryNodeState.LOCKED) {
            int routeCost = MasteryUiPolicy.minimumRouteCost(profile, node);
            if (routeCost > MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap()) {
                return Text.translatable("screen.simplyswordsmastery.node_state.budget", routeCost,
                        MasteryProfileRegistry.client().policy(profile.progressionGroupId()).pointCap());
            }
            if (!state.unlockedNodeIds().containsAll(node.requires())) {
                return Text.translatable("screen.simplyswordsmastery.node_state.prerequisites", node.cost());
            }
            return Text.translatable("screen.simplyswordsmastery.node_state.points",
                    node.cost(), state.availablePoints(profile));
        }
        return Text.translatable("screen.simplyswordsmastery.node_state." + nodeState.name().toLowerCase(), node.cost());
    }

    private static Text prerequisiteText(MasteryState state, MasteryProfile.Node node) {
        if (node.requires().isEmpty()) {
            return Text.translatable("screen.simplyswordsmastery.narration.prerequisites.none");
        }
        long met = node.requires().stream().filter(state.unlockedNodeIds()::contains).count();
        return Text.translatable("screen.simplyswordsmastery.narration.prerequisites", met, node.requires().size());
    }
