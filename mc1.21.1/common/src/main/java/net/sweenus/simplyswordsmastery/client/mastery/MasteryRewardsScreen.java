...
import net.sweenus.simplyswordsmastery.mastery.network.RewardActionPacket;
...
public final class MasteryRewardsScreen extends Screen {
    ...
    public MasteryRewardsScreen(RunicForgeScreenHandler handler, Screen parent) {
        super(Text.translatable("screen.simplyswordsmastery.rewards"));
    ...
    private void rebuild() {
        ...
        backButton = addDrawableChild(new GlassButtonWidget(frame.backButtonX(), buttonY,
                frame.backButtonWidth(), buttonHeight, Text.translatable("screen.simplyswordsmastery.back"),
        ...
        closeButton = addDrawableChild(new GlassButtonWidget(frame.closeButtonX(), buttonY,
                frame.closeButtonWidth(), buttonHeight, Text.translatable("screen.simplyswordsmastery.close"),
                button -> closeAll(), this::palette, accent, true));
        prevButton.setTooltip(Tooltip.of(Text.translatable("screen.simplyswordsmastery.rewards.prev")));
        nextButton.setTooltip(Tooltip.of(Text.translatable("screen.simplyswordsmastery.rewards.next")));
        applyPagerState();
        ...
        for (int i = 0; i < visible; i++) {
            ...
            GlassButtonWidget claim = addDrawableChild(new GlassButtonWidget(
                    ...
                    claimWidth, claimHeight, Text.translatable("screen.simplyswordsmastery.rewards.claim"),
    ...
    private void drawHeader(DrawContext context, int accent) {
        ...
        if (roomy) {
            MasteryChrome.microLabel(context, textRenderer,
                    Text.translatable("screen.simplyswordsmastery.rewards.kicker"), textLeft, titleTop,
        ...
        if (pages > 1) {
            ...
            UiDraw.centeredText(context, textRenderer,
                    Text.translatable("screen.simplyswordsmastery.rewards.page", page + 1, pages),
    ...
    private void drawBody(DrawContext context, int accent, int mouseX, int mouseY, float step, boolean instant) {
        ...
        int xpY = stripY + 17;
        MasteryChrome.microLabel(context, textRenderer,
                Text.translatable("screen.simplyswordsmastery.rewards.xp"), contentLeft, xpY,
                palette.INK_MUTED, alpha);
        Text xpValue = capped
    ...
    private void drawRow(DrawContext context, int accent, NbtCompound row, int index, float bodyAlpha) {
        ...
        Text amount = row.getInt("points") > 0
                ? Text.translatable("screen.simplyswordsmastery.rewards.amount.points", row.getInt("points"))
                : Text.translatable("screen.simplyswordsmastery.rewards.amount.xp", row.getInt("xp"));
        Text detail = claimable
                ? Text.literal(row.getString("cost"))
                : Text.translatable("screen.simplyswordsmastery.rewards." + row.getString("reason"));
    ...
    private void drawEmptyState(DrawContext context, float alpha) {
        ...
        UiDraw.centeredText(context, textRenderer,
                UiDraw.fit(textRenderer, Text.translatable("screen.simplyswordsmastery.rewards.empty"),
        ...
        UiDraw.Fitted hint = UiDraw.fitBlock(textRenderer,
                Text.translatable("screen.simplyswordsmastery.rewards.empty.hint"),
    ...
    private void drawShowcase(DrawContext context, int accent, float step, boolean instant) {
        ...
        MasteryShowcase.Hover hover = MasteryShowcase.draw(context, textRenderer, palette, accent,
                ...
                identityStack(), Text.translatable("profile.simplyswordsmastery." + profile.id().getPath()),
    ...
    private void drawStatus(DrawContext context, int accent) {
        ...
        if (!feedback.isEmpty() && MasteryChrome.feedback(context, textRenderer, palette, frame,
                Text.translatable("screen.simplyswordsmastery.rewards." + feedback),
        ...
        MasteryScreenSwitch.run(handler, () -> {
            if (MasteryProfileRegistry.resolveClient(identityStack()).isPresent()) {
                ...
                client.setScreen(new RunicForgeScreen(handler,
                        client.player.getInventory(), Text.translatable("screen.simplyswordsmastery.back")));
            }
        });
    }
}
