...
public abstract class AdvancementRewardMixin {
    ...
    @Unique private boolean simplyswordsmastery$loading;

    @WrapMethod(method = "load")
    private void simplyswordsmastery$load(ServerAdvancementLoader loader, Operation<Void> original) {
        boolean previous = simplyswordsmastery$loading;
        simplyswordsmastery$loading = true;
        try { original.call(loader); }
        finally { simplyswordsmastery$loading = previous; }
    }

    @WrapMethod(method = "grantCriterion")
    private boolean simplyswordsmastery$grant(AdvancementEntry advancement, String criterion, Operation<Boolean> original) {
        if (simplyswordsmastery$loading) return original.call(advancement, criterion);
