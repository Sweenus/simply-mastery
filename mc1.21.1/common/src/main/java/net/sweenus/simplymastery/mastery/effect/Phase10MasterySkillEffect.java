...
final class Phase10MasterySkillEffect implements AbilitySkillEffectType {
    ...
    private static Phase10AbilityTuning warglaive(Phase10AbilityTuning t, int branch, int slot) {
        ...
        if (branch == 1) return switch (slot) {
            ...
            case 6 -> t.with(s("SECONDARY_DAMAGE_MULTIPLIER"), .4);
        ...
        return switch (slot) {
            ...
            case 2 -> t.with(s("HEALTH_THRESHOLD"), .45);
            case 3 -> t.with(s("PER_STACK_MULTIPLIER"), .04).with(s("STACK_CAP"), 7);
            case 4 -> t.with(s("ABSORPTION"), 8);
            case 5 -> t.with(s("TARGET_CAP"), 3).with(s("STATUS_DURATION_TICKS"), 60);
            case 6 -> t.with(s("REFUND_TICKS"), 15).with(s("STACK_CAP"), 60);
    ...
    private static Phase10AbilityTuning ribbon(Phase10AbilityTuning t, int branch, int slot) {
        if (branch == 0) return switch (slot) {
            ...
            case 5 -> t.with(s("HEALTH_THRESHOLD"), .35).with(s("SPEED"), 1);
            case 6 -> t.with(s("COUNT"), 3).with(s("ABSORPTION"), 4).with(s("STATUS_DURATION_TICKS"), 60);
            case 7 -> t.with(s("SPEED"), .88).with(s("INCOMING_MULTIPLIER"), .7);
            case 8 -> t.with(s("SPEED"), 1).with(s("INCOMING_MULTIPLIER"), 1)
                    .with(s("OUTGOING_MULTIPLIER"), .88);
            default -> t;
        };
        if (branch == 1) return switch (slot) {
            ...
            case 7 -> t.multiply(s("RANGE"), 2, 1).with(s("TARGET_CAP"), 8)
                    .with(s("DAMAGE_MULTIPLIER"), .45).with(s("ANGLE"), 0);
            case 8 -> t.with(s("RANGE"), 5).add(s("COOLDOWN_TICKS"), 20, 40);
            default -> t;
        };
        return switch (slot) {
            case 0 -> t.with(s("DURATION_TICKS"), 80);
            case 1 -> t.add(s("DAMAGE_MULTIPLIER"), .15, .95);
            case 2 -> t.with(s("TARGET_CAP"), 2).with(s("RADIUS"), 2.5).with(s("SECONDARY_DAMAGE_MULTIPLIER"), .4);
            case 3 -> t.with(s("ARMOR_IGNORE"), .12);
            case 4 -> t.with(s("STATUS_DURATION_TICKS"), 40).with(s("STATUS_AMPLIFIER"), 1);
            case 5 -> t.with(s("WINDUP_TICKS"), 40).add(s("DAMAGE_MULTIPLIER"), .2, .95);
    ...
    private static Phase10AbilityTuning riftmane(Phase10AbilityTuning t, int branch, int slot) {
        if (branch == 0) return switch (slot) {
            ...
            case 6 -> t.with(s("REFUND_TICKS"), 30).with(s("RADIUS"), 6)
    ...
    private static Phase10AbilityTuning dawnquiver(Phase10AbilityTuning t, int branch, int slot) {
        if (branch == 0) return switch (slot) {
            ...
            case 6 -> t.with(s("COUNT"), 4).with(s("DELAY_TICKS"), 6).with(s("SECONDARY_DAMAGE_MULTIPLIER"), .55);
            case 7 -> t.with(s("COUNT"), 3).with(s("DAMAGE_MULTIPLIER"), .45);
            case 8 -> t.with(s("TARGET_CAP"), 1).multiply(s("DAMAGE_MULTIPLIER"), 1.85, 1)
        ...
        if (branch == 1) return switch (slot) {
            ...
            case 2 -> t.add(s("PITY_CHANCE"), 15, 20);
            case 3 -> t.with(s("REFUND_TICKS"), 1).with(s("CHANCE"), 20);
            case 4 -> t.with(s("ABSORPTION"), 4).with(s("STATUS_DURATION_TICKS"), 40).with(s("LOCKOUT_TICKS"), 40);
            case 5 -> t.with(s("PER_STACK_MULTIPLIER"), .03).with(s("STACK_CAP"), 6);
            case 6 -> t.with(s("COOLDOWN_TICKS"), 80);
        ...
        return switch (slot) {
            ...
            case 8 -> t.with(s("COUNT"), 3).with(s("ANGLE"), 12).with(s("DAMAGE_MULTIPLIER"), .55)
                    .multiply(s("COOLDOWN_TICKS"), 1.25, 200);
            default -> t;
        };
    }
    ...
    private static Phase10AbilityTuning dreadtide(Phase10AbilityTuning t, int branch, int slot) {
        if (branch == 0) return switch (slot) {
            ...
            case 6 -> t.with(s("HEALTH_THRESHOLD"), .3).with(s("LOCKOUT_TICKS"), 200);
            case 7 -> t.with(s("STACK_CAP"), 3).with(s("INCOMING_MULTIPLIER"), .8).with(s("SPEED"), 0);
            case 8 -> t.with(s("STACK_CAP"), 7).with(s("INCOMING_MULTIPLIER"), .95).with(s("SPEED"), .08);
            default -> t;
        };
        if (branch == 1) return switch (slot) {
            case 0 -> t.add(s("DURATION_TICKS"), 30, 100);
            case 1 -> t.multiply(s("DAMAGE_MULTIPLIER"), 1.1, 1);
            case 2 -> t.add(s("INTERVAL_TICKS"), -1, 10);
            ...
            case 7 -> t.multiply(s("DURATION_TICKS"), 2, 100).multiply(s("INTERVAL_TICKS"), .5, 10)
        ...
        return switch (slot) {
            ...
            case 3 -> t.with(s("CORRUPTION"), 60).with(s("INCOMING_MULTIPLIER"), .92);
            case 4 -> t.with(s("COUNT"), 5);
            case 5 -> t.with(s("ABSORPTION"), 4).with(s("TARGET_CAP"), 12);
            case 6 -> t.with(s("CORRUPTION"), 80).multiply(s("INTERVAL_TICKS"), .7, 60)
                    .with(s("INCOMING_MULTIPLIER"), 1.08);
            case 7 -> t.with(s("CORRUPTION"), 75).multiply(s("DAMAGE_MULTIPLIER"), 1.25, 1)
                    .with(s("ABSORPTION"), 8);
    ...
    private static boolean matches(int profile, int branch, UniqueAbilityDefinition definition) {
        if (profile == 4) return definition == Phase10UniqueAbilities.DREAD_CLOAK
                || branch == 1 && definition == Phase10UniqueAbilities.DREAD_ASSAULT;
        return definition == switch (profile) {
            case 0 -> branch == 0 ? Phase10UniqueAbilities.WARG_MARK : Phase10UniqueAbilities.WARG_HUNT;
            ...
            case 2 -> branch == 0 ? Phase10UniqueAbilities.RIFTMANE_HARRIER : Phase10UniqueAbilities.RIFTMANE_RANK;
