...
final class MasteryNodeAuditCatalog {
    ...
    private static Map<String, MasteryNodeAuditReport.Evidence> evidence() {
        ...
        verifiedProfile(evidence, "twisted_blade",
                new String[]{"twisted_blade/ferocity", "twisted_blade/crescendo", "twisted_blade/finale"},
                "Ferocity, Crescendo and Grand Crescendo", "TWISTED_", "TwistedBladeAbilityManager",
                new String[]{
                        "Blood Rhythm adds eight points to the configured Ferocity chance", "Lasting Fury adds thirty ticks to the configured Ferocity duration",
                        "Rapid Temper adds one percentage point to configured attack speed per stack", "Rising Heat uses a per-wielder five-hit counter with lifecycle cleanup",
                        "Unbroken Cadence has independent threshold, increment and bonus-cap settings", "Frenzied Footwork is maintained only while Ferocity is at least eight stacks",
                        "Overwound adds three to the live stack maximum and grants exactly five percent per overflow stack", "Endless Measure uses its ten-stack override and tuned forty-tick attack window",
                        "Fever Pitch uses its twenty-stack, low-health double-gain and eighty-tick shared-expiry profile", "Strong Beat composes multiplicatively with both Crescendo capstones",
                        "Wider Chord adds point four to the configured radius", "Accelerando subtracts one from the configured base interval while preserving the configured minimum",
                        "Driving Rhythm adds point fifteen to configured knockback", "Syncopation advances only on primary Crescendos and reads its interval, damage and pull settings",
                        "Resonant Wound uses successful hits, a sixty-tick window and bounded target records", "Grand Crescendo snapshots a four-tick secondary at sixty percent without advancing Syncopation",
                        "Soloist searches five blocks around the wielder, selects one nearest target and composes its damage", "Orchestra composes configured radius and damage before applying its target cap and knockback suppression",
                        "Patient Blade adds thirty ticks to the configured Finale window", "Loaded Chord multiplies only minimum Finale scaling on its dedicated channel",
                        "Full Measure multiplies only maximum Finale scaling on its dedicated channel", "Sweeping Finale adds point five to the interpolated live radius",
                        "Staggering Finish adds point two to the interpolated live knockback", "Encore detects any Grand Crescendo kill and adds four bounded Ferocity after twenty ticks",
                        "Perfect Timing has an independent twenty-tick window and damage multiplier", "Shattering Finale consumes all stacks for maximum scaling with a five-block, sixteen-target, forty-tick profile",
        ...
        verifiedProfile(evidence, "shadowsting",
                ...
                new String[]{
                        ...
                        "Quick Reflection subtracts one tick from the configured three-tick clone delay",
                        "Umbral Mark has its own three-second duration and 1.08 melee multiplier, and expired marks are pruned",
                        "Lingering Shade reads its own twelve-tick veil and now ends on the wielder's next attack",
                        "Twin Image reads its own fourth-proc interval, five-tick delay and fifty-percent multiplier",
                        "Hungry Darkness reads its own twenty-tick kill refund",
                        "Mirror Army composes count, damage and a separate fifteen-point chance penalty instead of overwriting Eager Shadow",
                        "Flawless Echo composes its 1.25 multiplier with Sharp Echo and holds its own sixty-tick lockout",
                        "Second Act adds twenty ticks to the configured fifty-tick dance; its add against a literal 100 was adding seventy",
                        "Quickstep subtracts one tick with its own three-tick floor; its add against a literal 8 was lengthening the interval from four to seven",
                        "Grand Stage adds two blocks to the configured strike radius on a dedicated channel",
                        "Cruel Choreography reads its per-step bonus and five-step cap instead of hardcoding them",
                        "Low Spotlight reads its own wounded-target threshold",
                        "Dancer's Guard grants Resistance I for the live dance and return on its own amplifier channel",
                        "Final Flourish reads its own radius, damage and target cap; its RADIUS 3 was shrinking the ten-block search and its DAMAGE_MULTIPLIER .6 was cutting every strike",
                        "Danse Macabre composes sixty extra ticks and two extra interval ticks against configuration and multiplies only the dance channel",
                        "Assassin's Waltz halves the live duration instead of rewriting the configured fifty it already equalled, and multiplies only the dance channel",
                        "Stray Step adds exactly one block to the 1.35-block arrival ring; its add against a literal 3 was landing arrivals 5.35 blocks out",
                        "Smoke Arrival reads its own radius, Blindness duration and target cap",
                        "Anchor Snare has independent radius, duration, amplifier and target cap and no longer collides with Smoke Arrival",
                        "Shadow Anchor subtracts two ticks from the eight-tick return without retiming Nocturne or Vanishing Point",
                        "Disorient reads its own Slowness duration and amplifier",
                        "Vanishing Point reads its own Speed duration and amplifier and no longer lengthens the return translation",
                        "Dark Reprieve reads its own interval, absorption grant and twelve-point cap",
                        "Nocturne reads its own three-second invisibility and no longer zeroes the arrival radius",
                        "Killing Step reads its own Resistance duration, amplifier and two skipped strikes"});
        verifiedProfile(evidence, "bloodwake",
                ...
                new String[]{
                        ...
                        "Deep Rupture multiplies only the burst damage channel",
                        "Contagious Blood adds forty ticks to Bleed's configured 160; its add against a literal 100 was shortening it to 140",
                        "Frenzied Feast reads its own second-burst interval and Frenzy grant",
                        "Hemorrhage applies its twenty-five percent only under its own threshold; its absolute multiplier was boosting every burst and discarding Deep Rupture",
                        "Chain Rupture reads its own delay, damage and four-detonation cap; its TARGET_CAP was capping the burst itself at four enemies",
                        "Arterial Spray has a real seven-block cone reach, angle and twelve-target cap and composes its damage with Deep Rupture",
                        "Blood Sacrament composes its radius and damage multipliers against live values and reads its Frenzy and absorption grants",
                        "Cheap Offering subtracts two ticks from the configured ten-tick rite cooldown; its add against a literal 20 was raising it to eighteen",
                        "Crimson Wake adds three steps to the configured wave length on a dedicated channel",
                        "Blood Scream adds two targets to the configured six and the Frenzy rank follows the live cap",
                        "Judgment Blades subtracts six ticks from the configured thirty-tick hover; its add against a literal 24 was resolving to eighteen",
                        "Carrion Host adds two flies over the configured eight under its own twelve-fly cap",
                        "Red Tide subtracts three ticks from the configured fifteen with its own eight-tick floor",
                        "Ritual Memory reads its own rite count, fifteen-second window and Frenzy grant",
                        "Ascending Rite adds thirty ticks to the configured cooldown from a stable base and its reduction reaches both damaging rites",
                        "Crimson Choice cycles through UniqueWeaponSecondaryAction so the sneak input costs no mana or cooldown, and its reduction reaches both damaging rites",
                        "Lasting Stain adds one hundred ticks to the configured six-hundred-tick stain",
                        "Thick Blood adds one Slowness rank over configuration under its own Slowness II cap",
                        "Sanguine Renewal subtracts eight ticks from the configured forty with its own twenty-tick floor",
                        "Rich Ground adds exactly half a heart; its COUNT 1 was resolving to three and adding 1.5",
                        "Bloodbound reads its own knockback-resistance value and is released on world, actor and server cleanup",
                        "Red Footing reads its own 1.08 multiplier and its text no longer promises wave damage",
                        "Confluence reads its forty-second merge cap instead of writing it to the base stain duration",
                        "Sea of Blood reads its own width multiplier and adds two hundred ticks instead of overwriting Lasting Stain",
                        "Heartpool reads its own width, heal multiplier, absorption and four-second absorption cadence instead of retiming the heal"});
